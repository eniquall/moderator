...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
    ...
    /**
     * @var boolean whatever to check and create non existing collections
     */
    protected static $ensureMongoStructure = false;
     ...
     * @since v1.0
     */
    public function getDb($server = 'master') {
        return $this->getMongoDBComponent()->getDbInstance($server);
    }
    ...
    public function getCollectionName()
    {
        ...
        if (!is_array($structure) || !isset($structure['name'])) {
            throw new CDbException('Collection structure is not defined!');
        }
     ...
     * )
     */
    abstract public function getCollectionStructure();
     ...
     * @since v1.0
     */
    public function getCollection($server = 'master') {
        if (!isset(self::$_collections[$this->getCollectionName()]))
            self::$_collections[$this->getCollectionName()] = $this->getDb($server)->selectCollection($this->getCollectionName());
    ...
    public function setAttributes($values, $safeOnly = true) {
        ...
        yii::trace(mb_substr(print_r($values, true), 0, 150).'...', 'ext.ymds');
    ...
    public function init() {
        ...
        if (!static::$ensureMongoStructure) {
            $this->ensureIndexes = false;
        } else {
           
            $cmd = array_merge(
                array(
                    'create' => $this->getCollectionName()
                ),
                $this->getCollectionStructure()
            );
                    
            $this->getDb()->command($cmd);
        }
    ...
    public function recreateCollection()
    {
        unset(self::$_indexes[$this->getCollectionName()]);
        $this->getCollection()->drop();
        self::ensureMongoStructure(true);
        $this->init();
        self::ensureMongoStructure(false);
    }
    
    public static function ensureMongoStructure($ensure)
    {
        static::$ensureMongoStructure = (bool) $ensure;
    }
     ...
     * @since v1.0
     */
    public function refresh($server = 'slave') {
        Yii::trace(get_class($this) . '.refresh()', 'ext.MongoDb.EMongoDocument');
        if (!$this->getIsNewRecord() && $this->getCollection($server)->count(array('_id' => $this->_id)) == 1) {
            $this->setAttributes($this->getCollection($server)->findOne(array('_id' => $this->_id)), false);
            return true;
        }
        else
            return false;
    ...
    public function aggregate(array $options = array())
    {
        ...
        if (!empty($this->_cacheDuration) && ($cacheComponent = $this->getCacheComponent())) {
            $duration = $this->_cacheDuration;
            $this->_cacheDuration = null;
            $cacheHash = md5(print_r($options, true));
        
            if ($cache = $cacheComponent->get($cacheHash)) {
                $out = unserialize($cache);
                Yii::trace('Cache Group hits ' . get_class($this) . print_r($out, true));
            } else {
                $out = $this->getCollection()->aggregate($options);
                if (!empty($out['ok'])) {
                    $cacheComponent->set($cacheHash, serialize($out), $duration);
                }
            }
        } else {
            $out = $this->getCollection()->aggregate($options);
     ...
     * @since v1.0
     */
    public function find($criteria = null, $server = 'slave') {
        Yii::trace(get_class($this) . '.find() with criteria:' . print_r($criteria, true), 'ext.MongoDb.EMongoDocument');
        
        if ($this->beforeFind()) {
            $this->applyScopes($criteria);
            Yii::beginProfile('YMDS.find');
            
            if (!empty($this->_cacheDuration) && ($cacheComponent = $this->getCacheComponent())) {
                $duration = $this->_cacheDuration;
                $this->_cacheDuration = null;
                $cacheHash = $this->_buildCacheHash($criteria);
                
                if ($cache = $cacheComponent->get($cacheHash)) {
                    $doc = unserialize($cache);
                    Yii::trace('Cache hits ' . get_class($this) . print_r($doc, true));
                } else {
                    $doc = $this->getCollection($server)->findOne($criteria->getConditions(), $criteria->getSelect());
                    if (!empty($doc)) {
                        $cacheComponent->set($cacheHash, serialize($doc), $duration);
                    } 
                }
            } else {
                $doc = $this->getCollection($server)->findOne($criteria->getConditions(), $criteria->getSelect());
     ...
     * @since v1.0
     */
    public function findAll($criteria = null, $server = 'slave') {
        ...
        if ($this->beforeFind()) {
            ...
            if ($this->getUseCursor($criteria)) {
                ...
                if (!empty($this->_cacheDuration) && ($cacheComponent = $this->getCacheComponent())) {
                    ...
                    if ($cache = $cacheComponent->get($cacheHash)) {
                        $docs = unserialize($cache);
                        Yii::trace('Cache findAll hits ' . get_class($this) . print_r($docs, true));
                    } else {
                        $cursor = $this->_findAll($criteria);
                        $docs = array();
                        ...
                        if (!empty($docs)) {
                            $cacheComponent->set($cacheHash, serialize($docs), $duration);
                        }
                    }
                } else {
                    $docs = $this->_findAll($criteria);
                }
    ...
    private function _findAll($criteria = null)
    {
        Yii::beginProfile('YMDS.findAll');
        $cursor = $this->getCollection()->find($criteria->getConditions());
        Yii::endProfile('YMDS.findAll');
        ...
        if (YII_DEBUG) {
            $explain = $this->getCollection()->find($criteria->getConditions())->explain();
            Yii::trace("Condition:" . print_r($criteria->getConditions(), true) . ' Explain:' . print_r($explain, true), 'ext.MongoDb.EMongoDocument.explain');
        }
     ...
     * @since v1.0
     */
    public function findByPk($pk, $criteria = null, $server = 'slave') {
        Yii::trace(get_class($this) . '.findByPk()', 'ext.MongoDb.EMongoDocument');
        $class = get_class($this); 
        Yii::beginProfile("YMDS.{$class}.findByPk");
        $criteria = new EMongoCriteria($criteria);
        $criteria->mergeWith($this->createPkCriteria($pk));

        $result = $this->find($criteria, $server);
        Yii::endProfile("YMDS.{$class}.findByPk");
     ...
     * @since v1.0
     */
    public function findByAttributes(array $attributes, $server = 'slave') {
        $criteria = new EMongoCriteria();
        foreach ($attributes as $name => $value) {
            $criteria->$name('==', $value);
        }

        return $this->find($criteria, $server);
    }
     ...
     * @since v1.0
     */
    public function findAllByAttributes(array $attributes, $server = 'slave') {
        $criteria = new EMongoCriteria();
        foreach ($attributes as $name => $value) {
            $criteria->$name('==', $value);
        }
     ...
     * @since v1.0
     */
    public function count($criteria = null, $server = 'slave') {
        Yii::trace(get_class($this) . '.count()', 'ext.MongoDb.EMongoDocument');

        $this->applyScopes($criteria);
        
        Yii::beginProfile('YMDS.count');
        $res = $this->getCollection($server)->count($criteria->getConditions());
        Yii::endProfile('YMDS.count');
        return $res;
     ...
     * @since v1.2.2
     */
    public function countByAttributes(array $attributes, $server = 'slave') {
        ...
        return $this->count($criteria, $server);
    }
    ...
    private function _buildCacheHash(EMongoCriteria $criteria)
    {
        return md5 (
            print_r($criteria->getConditions(), true) .
            print_r($criteria->getSelect(), true) .
            intval($criteria->getLimit()) .
            intval($criteria->getOffset()) .
            print_r($criteria->getSort(), true) 
        );
    }
    ...
    {
        $cacheId = $this->getMongoDBComponent()->cacheId;
        $cacheComponent = Yii::app()->getComponent($cacheId);
        
        if (!empty($cacheId) && empty($cacheComponent)) {
            Yii::log('Not found cache component ' . $cacheId, CLogger::LEVEL_ERROR);
        }
